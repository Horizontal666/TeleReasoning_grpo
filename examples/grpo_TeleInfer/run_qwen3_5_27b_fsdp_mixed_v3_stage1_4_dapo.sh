...
#     27B FSDP + telelogs_symbolic reward stack we already smoke-tested.
...
#     gen_batch_size, dynamic_bsz).
#
# DAPO-specific deltas vs the GRPO sibling:
...
#   3. Clip-Higher: clip_ratio_low=0.20, clip_ratio_high=0.28, clip_ratio_c=10
...
#   7. actor.use_dynamic_bsz=True + ppo_max_token_len_per_gpu=24576
#   8. ref.log_prob_use_dynamic_bsz=True + log_prob_max_token_len_per_gpu=32768
#   9. reward_model.overlong_buffer.enable=False (matches v0.7)
#
# v0.8 DAPO recipe required 3 upstream patches to run at all (recorded inline
# in those files):
#   - recipe/dapo/main_dapo.py     : instantiate legacy BatchRewardManager
#                                    directly (load_reward_manager signature
#                                    changed; experimental managers don't have
#                                    sync __call__).
#   - verl/trainer/ppo/reward.py   : add `compute_reward` shim (removed in a
#                                    refactor, still imported by dapo_ray_trainer).
#   - verl/models/transformers/monkey_patch.py : guard trl >=0.20 missing
#                                    AutoModelForCausalLMWithValueHead.
#
# Usage:
#   bash examples/grpo_TeleInfer/run_qwen3_5_27b_fsdp_mixed_v3_stage1_4_dapo.sh
#   EXPERIMENT_NAME=dapo_v3 STAGE_NAME=stage1.4 bash examples/grpo_TeleInfer/run_qwen3_5_27b_fsdp_mixed_v3_stage1_4_dapo.sh
...
TELEMATH_TEST=/dpc/kuin0100/bohao/202509_InferenceModel/data/GRPO/telemath/test.parquet
...
########################### DAPO knobs (matches v0.7 run_stage_mixed_v3_dapo.sh)
...
# Our shim exposes both compute_score_batched (legacy batched) and
# compute_score (per-sample). DAPO's RayDAPOTrainer uses the legacy
# BatchRewardManager which expects the *batched* function.
CUSTOM_REWARD_PATH=${CUSTOM_REWARD_PATH:-${SCRIPT_DIR}/telelogs_symbolic_reward.py}
CUSTOM_REWARD_NAME=${CUSTOM_REWARD_NAME:-compute_score_batched}
...
if [[ "${NO_VLLM_ATTENTION_BACKEND:-0}" != "1" ]]; then
...
echo "[INFO] DAPO: filter_groups.metric=${filter_groups_metric} clip=(${clip_ratio_low},${clip_ratio_high}) loss_agg=${loss_agg_mode} n=${rollout_n} gen_bsz=${gen_batch_size}"
