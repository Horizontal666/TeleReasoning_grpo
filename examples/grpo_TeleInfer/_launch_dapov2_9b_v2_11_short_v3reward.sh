#!/usr/bin/env bash
# DAPO with v3 (process-aware) reward on Qwen3.5-9B SFT v2.11_short.
# Auto-detects latest merged v2.11_short checkpoint.
...
INIT_MODEL=$(ls -d $MERGED_BASE/Qwen3.5-9B-TelecomInstruct_v2.11_short_qlora_stage3_checkpoint-* 2>/dev/null | sort -V | tail -1)
if [ -z "$INIT_MODEL" ]; then
    echo "[launcher] ERROR: no v2.11_short merged checkpoint found in $MERGED_BASE" >&2
    exit 1
fi
...
export EXPERIMENT_NAME=dapo_mixed_v3_stage2_0_grpoeval_qwen35_9b_v2_11_short_${CKPT_NAME}_v3rs
...
echo "[launcher] THREEGPP_REWARD_MODE=v3 (STRONG, anti_reject_gt=0.30)"
