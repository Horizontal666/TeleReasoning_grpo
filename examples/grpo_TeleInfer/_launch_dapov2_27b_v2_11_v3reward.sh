#!/usr/bin/env bash
# DAPO with v3 (process-aware) reward on Qwen3.5-27B SFT v2.11.
# Auto-detects latest merged v2.11 27B checkpoint.
...
INIT_MODEL=$(ls -d $MERGED_BASE/Qwen3.5-27B-TelecomInstruct_v2.11_qlora_stage3_checkpoint-* 2>/dev/null | sort -V | tail -1)
if [ -z "$INIT_MODEL" ]; then
    echo "[launcher] ERROR: no v2.11 27B merged checkpoint found in $MERGED_BASE" >&2
    exit 1
fi
...
export EXPERIMENT_NAME=dapo_mixed_v3_stage2_0_grpoeval_qwen35_27b_v2_11_${CKPT_NAME}_v3rs
...
export MAX_CKPT=${MAX_CKPT:-1}  # 2026-05-30: keep only latest 1; top-2 by best-2 saver
