...
# Respect externally provided GPU selection and default to an 8x H200 layout.
...
# Tune defaults by model size so 8x H200 140G can run a higher-throughput
# configuration out of the box, while still allowing every knob to be overridden
# from the shell.
...
if [[ "${MODEL_BASENAME}" == *235B* || "${MODEL_BASENAME}" == *A22B* || "${MODEL_BASENAME}" == *70B* || "${MODEL_BASENAME}" == *72B* ]]; then
    ...
    DEFAULT_ACTOR_PPO_MICRO_BATCH_SIZE_PER_GPU=1
    DEFAULT_ROLLOUT_N=4
    ...
    DEFAULT_LOGPROB_MAX_TOKEN_LEN_PER_GPU=32768
    DEFAULT_ACTOR_PPO_MAX_TOKEN_LEN_PER_GPU=32768
    ...
    DEFAULT_PPO_MINI_BATCH_SIZE=256
    DEFAULT_ACTOR_PPO_MICRO_BATCH_SIZE_PER_GPU=1
    DEFAULT_ROLLOUT_N=4
    ...
    DEFAULT_ENABLE_PREFIX_CACHING=False
    DEFAULT_LOGPROB_MAX_TOKEN_LEN_PER_GPU=49152
    DEFAULT_ACTOR_PPO_MAX_TOKEN_LEN_PER_GPU=49152
    ...
    DEFAULT_TRAIN_BATCH_SIZE=384
    DEFAULT_PPO_MINI_BATCH_SIZE=384
    DEFAULT_ACTOR_PPO_MICRO_BATCH_SIZE_PER_GPU=1
    DEFAULT_ROLLOUT_N=4
    DEFAULT_GPU_MEMORY_UTILIZATION=0.90
    DEFAULT_ROLLOUT_MAX_BATCHED_TOKENS=49152
    ...
    DEFAULT_PPO_MINI_BATCH_SIZE=512
    DEFAULT_ACTOR_PPO_MICRO_BATCH_SIZE_PER_GPU=2
    DEFAULT_ROLLOUT_N=4
    DEFAULT_GPU_MEMORY_UTILIZATION=0.92
    DEFAULT_ROLLOUT_MAX_BATCHED_TOKENS=65536
    DEFAULT_ROLLOUT_MAX_NUM_SEQS=128
    DEFAULT_ENABLE_PREFIX_CACHING=True
    DEFAULT_LOGPROB_MAX_TOKEN_LEN_PER_GPU=65536
    DEFAULT_ACTOR_PPO_MAX_TOKEN_LEN_PER_GPU=65536
    DEFAULT_ACTOR_PARAM_OFFLOAD=False
    DEFAULT_ACTOR_OPTIMIZER_OFFLOAD=False
    DEFAULT_REF_PARAM_OFFLOAD=False
fi
...
ACTOR_PPO_MICRO_BATCH_SIZE_PER_GPU="${ACTOR_PPO_MICRO_BATCH_SIZE_PER_GPU:-${DEFAULT_ACTOR_PPO_MICRO_BATCH_SIZE_PER_GPU}}"
...
LOGPROB_MAX_TOKEN_LEN_PER_GPU="${LOGPROB_MAX_TOKEN_LEN_PER_GPU:-${DEFAULT_LOGPROB_MAX_TOKEN_LEN_PER_GPU}}"
ACTOR_PPO_MAX_TOKEN_LEN_PER_GPU="${ACTOR_PPO_MAX_TOKEN_LEN_PER_GPU:-${DEFAULT_ACTOR_PPO_MAX_TOKEN_LEN_PER_GPU}}"
RUN_TAG="${RUN_TAG:-}"
DRY_RUN="${DRY_RUN:-0}"
